...
# Neural Network
class CnnDigits2(nn.Module):
    def __init__(self, name, ch_in = 1, d_out = 10, size_im = [16, 16], n_patterns1 = 6,
                 n_patterns2 = 16, kernel_pool = 2, dtype=torch.float, device='cpu'):
        super(CnnDigits2, self).__init__()
        ...
        self.fc1 = torch.nn.Linear(int(n_patterns2 * size_im[0]/(kernel_pool*kernel_pool) * size_im[1]/(kernel_pool*kernel_pool)), 32)
